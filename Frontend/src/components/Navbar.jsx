...
// Bottom nav item styling (standard)
...
function Navbar() {
  ...
  // Determine bottom navigation items based on auth and role
  const getBottomNavItems = () => {
    const baseItems = [
      ...
      { to: '/menu', label: 'Menu', icon: 'M4 6h16M4 12h16M4 18h16', end: false },
    ]
    const authItems = isAuthenticated
      ? [
          { to: '/cart', label: 'Cart', icon: 'M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z', badge: totalQuantity > 0 ? totalQuantity : null },
          { to: user?.role === 'admin' ? '/admin' : '/profile', label: user?.role === 'admin' ? 'Admin' : 'Profile', icon: user?.role === 'admin' ? 'M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z' : 'M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z', end: false }
        ]
      : [
          { to: '/login', label: 'Login', icon: 'M11 16l-4-4m0 0l4-4m-4 4h14m-5 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h7a3 3 0 013 3v1', end: false },
          { to: '/register', label: 'Sign Up', icon: 'M18 9v3m0 0v3m0-3h3m-3 0h-3m-2-5a4 4 0 11-8 0 4 4 0 018 0zM3 20a6 6 0 0112 0v1H3v-1z', end: false }
        ]
    // Insert search item in the middle (index 2)
    const searchItem = {
      to: '/menu',
      label: 'Search',
    ...
    // Combine: home, menu, search, then auth items
    ...
    <>
      ...
      <nav className="sticky top-0 z-40 border-b border-orange-100 bg-white/90 backdrop-blur-md">
        <div className="container-fluid mx-auto px-4 py-3 flex items-center justify-between gap-4">
          ...
          {/* Mobile top bar: show user status instead of search */}
        ...
        {/* Desktop-only: no mobile search input */}
      </nav>
              ...
              return (
                ...
                >
                  <div className="relative">
                    {isSearch ? (
                      // Central search icon with theme color background
      ...
      {/* Add padding bottom to main content to avoid overlapping with bottom nav */}
      <style jsx>{`
